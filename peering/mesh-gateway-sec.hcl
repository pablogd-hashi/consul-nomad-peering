...
job "mesh-gateway" {
  ...
  group "mesh-gateway-one" {
    ...
    service {
      ...
      tagged_addresses {
        wan_ipv4 = "${attr.unique.platform.gce.network.dcanadillas-sec-network.external-ip.0}"
        wan = "${attr.unique.platform.gce.network.dcanadillas-sec-network.external-ip.0}"
      }
